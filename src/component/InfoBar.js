...
const LocationInfoBar = (props) => {
  ...
  useEffect(() => {
    fetch(`/getFavLoc?username=${username}`, {
      ...
      .then((data) => {
        if (data.favoritelist && data.favoritelist.includes(props.venueName)) setFavorite(true);
      });
  }, []);
  ...
  return (
    <div className="alert alert-info shadow-lg">
      ...
      <div>
        Add to Favorite
        <input type="checkbox" className={`toggle toggle-lg toggle-warning`} checked={Favorite} onClick={toggleFavorite} />
      </div>
    </div>
  );
};
